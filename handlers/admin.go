...
// WebhookDeploy handles deployment webhooks from GitHub Actions
func WebhookDeploy(w http.ResponseWriter, r *http.Request) {
	if r.Method != "POST" {
	...
	if expectedSecret == "" {
		expectedSecret = "default-secret" // fallback for development
	}

	if payload.Secret != expectedSecret {
		log.Printf("❌ Invalid webhook secret")
		http.Error(w, "Invalid webhook secret", http.StatusUnauthorized)
		return
	}

	log.Printf("🚀 Webhook deployment triggered for image: %s", payload.Image)

	// Execute deployment script
	cmd := exec.Command("/bin/bash", "webhook-deploy.sh", payload.Image, payload.Secret)
	cmd.Env = append(os.Environ(), "WEBHOOK_SECRET="+expectedSecret)
	...
	log.Printf("✅ Deployment successful: %s", string(output))
